...
export default props => {
  ...
  const education = EductationData.map((item, index) => (
    ...
    icon={<SchoolIcon />}
  >
    <h3 className="vertical-timeline-element-title">{(item.degree || "") + " " + (item.major ? "in " + item.major : "")}</h3>
    <h4 className="vertical-timeline-element-subtitle">{item.school} - {item.where}</h4>
    <p>
    {item.moreInfo}
    </p>
  </VerticalTimelineElement>
